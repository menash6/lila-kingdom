...
const ContaineraJumbutron = styled.div`
  ...
  background-image: ${({ typeBackground }) =>
    typeBackground === "homePage" ? `url(${backgroundImg1})` : `url(${backgroundImg2})`};
...
const Jumbutron = ({ type }) => {
  const homePage = (
    <>
      ...
      <div className="header-block">
        <h1>My name is Lila</h1>
        <h3>and I like to draw! 😊</h3>
      </div>
    </>
  );
  ...
  return (
    <ContaineraJumbutron typeBackground={type} name="start-position">
      <MainContainer>{type === "homePage" ? homePage : homePage}</MainContainer>
    </ContaineraJumbutron>
  );
};
